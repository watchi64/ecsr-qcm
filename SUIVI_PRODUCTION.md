...
- **Une affirmation invérifiable ne rentre pas dans un QCM** (règle utilisateur du 2026-07-30, verbatim : « si une question n'est pas vérifiable par l'ONISR ou d'autres sources officielles, vaut mieux ne pas la prendre. On risque de dire une connerie »). Quand un chiffre ne se retrouve ni à l'ONISR, ni sur Légifrance, ni sur un autre site officiel, on **supprime la question** au lieu de la publier avec une réserve. Un trou dans la couverture est préférable à une bêtise énoncée devant la promo. Deux nuances : quand le chiffre douteux est dans l'**énoncé** mais que la question porte sur autre chose, on retire le chiffre et on garde la question ; et une **notion pédagogique** sans texte réglementaire (les 5 mobiles de la prise de risque, l'homéostasie du risque) n'est pas concernée, ce n'est pas une statistique falsifiable, il suffit d'annoncer sa vraie source.
- **Un enonce se termine par une vraie question, avec un VERBE.** « Un conducteur double sur une ligne continue pour ne pas rater son train. Quel mobile ? » se lit comme une note de brouillon : l'eleve doit deviner ce qu'on lui demande. Ecrire « De quel mobile s'agit-il ? ». Le procede situation-puis-question est bon et tres present dans la base ; c'est le fragment nominal final qui ne va pas. Douze corriges le 2026-08-01 (« Quelle amende ? », « Quelle sanction ? », « Le risque ? », « Effet attendu ? », « Quel ecart lateral minimum ? »...).
- **Le terme savant va dans l'ENONCE, sa definition dans les reponses** (regle tiree de la relecture utilisateur du 2026-08-01). « Qu'appelle-t-on l'accommodation de l'oeil ? » suivi de « La mise au point du cristallin sur une distance » : l'eleve apprend le mot en lisant ce qu'il designe. Le sens inverse (enonce en francais courant, reponses en jargon nu) laisse l'eleve sans prise : il doit deja connaitre les quatre termes pour repondre. Quand on veut malgre tout faire nommer un phenomene, deux conditions : **ancrer l'enonce dans une situation concrete**, et **gloser CHAQUE option**, pas seulement la bonne, sinon la glose designe la reponse. Exemple applique : « Une cecite d'inattention, on regarde sans voir ».
- **Bannir les mots rares quand un mot courant dit la meme chose.** « Un conducteur mu par le risque-stimulation » est devenu « un conducteur qui prend un risque par stimulation ». Le vocabulaire de specialite se garde parce qu'il est au programme ; le vocabulaire litteraire, non.
- **Donner la formule enseignée, pas seulement le critère légal** (règle ajoutée le 29/07 sur remarque utilisateur). Quand une règle est transmise aux élèves sous forme d'une formule courte, l'explication doit la porter **en plus** du critère réglementaire : la formule est ce qui reste en tête, le critère est ce qui tranche les cas limites. Exemple appliqué : « sur terrain plat, c'est le plus manœuvrable qui passe » vient en tête d'explication, suivi du critère de R414-2 (plus de 2 m de large ou 7 m de long). Écrire l'un sans l'autre appauvrit la fiche : le critère seul ne se retient pas, la formule seule ne permet pas de trancher.
...
- **2026-07-31** : **PUBLICATION. Les 65 QCM sont en ligne pour la promo.** 1 057 questions, 5 226 options, 20 illustrees. Verifie en simulant un vrai compte stagiaire sous RLS : il voit les 65 QCM et leurs 1 057 questions, et aucun signalement des autres. Un reglage corrige juste avant la bascule : le QCM « La signalisation verticale » etait configure pour tirer **1 question sur 15** a l'examen, reste d'un test ; remis au defaut des 63 autres (toute la banque). Le QCM « Evaluation C1 » garde son tirage de 5 sur 20, qui est un choix plausible.
  ...
  Le filet de relecture est le **signalement d'erreur** : chaque eleve signale une question douteuse apres correction, le formateur la voit remonter avec un badge sur la liste des themes et un bouton sur la fiche QCM.

- **2026-07-31** : **LES VISUELS, PUIS LA LISIBILITE.** Signalement de l'utilisateur : « Que signifie un panneau rond entierement blanc barre de trois traits obliques noirs ? » transforme une lecture de panneau en devinette. **Le panneau se montre, il ne se decrit pas.** Huit questions passent a l'image, tirees des 833 SVG officiels de `ressources/` (le registre `signaux.js` evoque dans CLAUDE.md n'existe pas cote app) : B31, B1, B14, AB25, AB6, G1, plus les balises J10 d'approche de passage a niveau. Les codes `AB25` et `AB6` quittent les enonces : reconnaitre le panneau fait partie de l'exercice. La question sur la croix de Saint-Andre est **inversee** (elle demandait le nom d'un panneau, elle montre le panneau et demande ce qu'il signale).

  **Le marquage au sol a demande trois versions.** La bibliotheque ne contient que des panneaux, il a fallu les dessiner. Premiere version rejetee par l'utilisateur (« pas fou ce resultat la ») : les fleches pointaient vers le haut dans une vue ou la circulation va vers la droite. Deuxieme version refaite en vue verticale. **Troisieme version, la bonne** : l'utilisateur a fourni une planche de reference, et les cotes ont ete confirmees dans l'IISR 7e partie publiee par le Cerema. Cela a revele une **erreur de fond** : j'avais dessine la ligne de rive a un rapport trait/espacement de 0,76 alors que le T'1 officiel est a **1/3**. Les six modulations sont desormais tracees par calcul a 8 px par metre (T1 3/10, T'1 1,5/5, T2 3/3,5, T3 3/1,33, T'3 20/6, T4 39/13) et **mesurees sur le dessin genere**, pas verifiees a l'oeil. Le T4 a 39 m recoupe la question sur la bande d'arret d'urgence. Aucune image n'a ete copiee : seules les cotes, donnees reglementaires, ont ete reprises.

  **Fond gris derriere les visuels** (`#c6cbd1`), volontairement independant du theme : les balises J et les panneaux de fin d'interdiction sont blancs et disparaissaient sur fond clair. Valeur choisie par calcul de contraste (1,63 contre le blanc, 3,01 contre le rouge) et non a l'estime.

  **Purge du tiret cadratin dans la BASE** : la decision du 29/07 avait ete appliquee au depot (1 850 occurrences) mais **jamais aux donnees**. 74 occurrences restantes traitees selon le tableau de la charte : virgule pour les 70 explications (complement ou restriction), parentheses pour l'incise de l'enonce 196, trait d'union pour les 3 titres de QCM.

  **Passe de lisibilite** : enonces telegraphiques rendus interrogatifs (« N' est pas considere comme une voie reservee : » devient « Laquelle de ces voies n'est pas une voie reservee ? »), numeros d'article sortis des enonces vers les explications, definition juridique de 178 caracteres descendue dans l'explication. Verifie au passage : **« carrefour a l'indonesienne » est un vrai concept enseigne** (Wikipedia, supports d'auto-ecole), ma suspicion etait fausse.

  **Etat : 1 057 questions, 5 226 options, 20 illustrees.** Zero enonce sans ponctuation d'appel, zero article dans un enonce, zero biais de longueur, zero cadratin, zero artefact de production.

- **2026-07-29** : **QUESTIONS AMBIGUËS FAUTE DE CONTEXTE : la règle du croisement dépend de la déclivité.** L'utilisateur repère sur la question du croisement sur chaussée étroite (qcm 10, id 363) deux défauts : l'énoncé disait « trop étroite pour un croisement **facile** » sous une section intitulée « Croisements **difficiles** », et surtout **il ne précisait pas si la route était en pente**. Or les deux règles s'inversent, vérification PISTE faite : **R414-2** (en vigueur), sur route étroite, c'est le véhicule de plus de **2 m de large ou 7 m de long** (remorque comprise) qui s'efface, sauf transport en commun en agglomération ; **R414-3** (en vigueur), sur route de montagne ou à forte déclivité, c'est le **véhicule descendant** qui s'arrête le premier, **quel que soit son gabarit**. Le leurre « le véhicule qui monte » était donc l'inverse d'une vraie règle, et la question **contredisait ses propres voisines** (les questions 13 à 15 du même QCM enseignent la règle de montagne). Énoncés recentrés sur « route étroite **et sans forte déclivité** », et les deux explications se renvoient désormais l'une à l'autre pour que la paire s'apprenne ensemble. Un balayage a trouvé **un second cas identique** (id 860, qcm 56, semi-remorque contre voiture), corrigé de la même façon ; les autres occurrences de « facilité » relevaient du terme légal « VIG à facilités de passage », sans ambiguïté.

  **Angle mort de mes passes précédentes découvert au passage** : mes mesures du biais de longueur ne portaient que sur les questions à **réponse unique** (`nb_ok = 1`). Les **15 questions à réponses multiples** n'avaient jamais été contrôlées, et sur l'une d'elles (qcm 14, feu jaune clignotant), les **deux bonnes réponses étaient les deux options les plus longues**, ce qui livrait la solution complète à qui compte les caractères. Corrigée ; pire écart résiduel sur les 15 : **4 caractères**.
